function task1()
{
    ...
    {
        var id = faces[i];
        var S0 = Math.floor(255 * (scalars[ id[0] ] - 0.1) / 0.7);
        var S1 = Math.floor(255 * (scalars[ id[1] ] - 0.1) / 0.7);
        var S2 = Math.floor(255 * (scalars[ id[2] ] - 0.1) / 0.7);
...
function task2()
{
    ...
    // Draw color map
    var lut = new THREE.Lut( 'cooltowarm', cmap.length );
    lut.addColorMap( 'mycolormap', cmap );
    lut.changeColorMap( 'mycolormap' );
    scene.add( lut.setLegendOn( {
        'layout':'horizontal',
